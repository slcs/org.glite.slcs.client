 ...
 * @see Attributes
 */
public abstract class SequentialListSegment<T> extends Segment implements List<T> {
	...
	public T get(final int index) {
		final ListIterator<T> li=listIterator(index);
		try {
			return(li.next());
	...
	public Object[] toArray() {
		...
		int x=0;
		for (final ListIterator<T> li=listIterator(0); li.hasNext();) array[x++]=li.next();
		return array;
	}
	...
	public <U> U[] toArray(U a[]) {
		...
		if (a.length<count) a=(U[]) java.lang.reflect.Array.newInstance(a.getClass().getComponentType(),count);
		int x=0;
		for (final ListIterator<T> li=listIterator(); li.hasNext();)
			a[x++]=(U) li.next();
		if (a.length>count) 
			a[count]=null;
		return a;
	...
	public int indexOf(final Object o) {
		final ListIterator<T> li=listIterator(0);
		if (o==null) {
	...
	public int lastIndexOf(final Object o) {
		final ListIterator<T> li=listIterator(getCount());
		if (o==null) {
	...
	private static class SubList<T> extends AbstractList<T> {
		...
		SubList(final List<T> list, final int fromIndex, final int toIndex) {
		...
		public ListIterator<T> listIterator(final int index) {
			return new ListIterator<T>() {
				private final ListIterator<T> i=list.listIterator(getSuperListIndex(index));
